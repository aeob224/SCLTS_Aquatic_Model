...
data2021 <- df %>%
  filter(Year == 2021) %>%
  select(Pond, Metamorph_Estimate, Drift_Fence_Actual_Count)
...
plot1 <- ggplot(data2021, aes(x = log(Metamorph_Estimate+1), y = log(Drift_Fence_Actual_Count+1))) +
  ...
  labs(x = "log(Larval Density Derived Metamorph Prediction)",
       y = "log(Actual Metamorph Count)",
       title = "2021")+
  theme(plot.title = element_text(size = 30), axis.text = element_text(size = 16), axis.title = element_text(size = 18))+
  annotate("text", x = 0.1, y = 2.75, label = paste("R² = ", round(summary2021$r.squared, 3), "\n", "p = ", round(p_value2021, 3)),
           size = 5, color = "black")
...
plot2 <- ggplot(data2022, aes(x = log(Metamorph_Estimate+1), y = log(Drift_Fence_Actual_Count+1))) +
  ...
  labs(x = "log(Larval Density Derived Metamorph Prediction)",
       ...
       title = "2022")+
  theme(plot.title = element_text(size = 30), axis.text = element_text(size = 16), axis.title = element_text(size = 18))+
  annotate("text", x = 0.1, y = 5.9, label = paste("R² = ", round(summary2022$r.squared, 3), "\n", "p = ", round(p_value2022, 3)),
           size = 5, color = "black")
...
plot3 <- ggplot(data2023, aes(x = log(Metamorph_Estimate+1), y = log(Drift_Fence_Actual_Count+1))) +
  ...
  labs(x = "log(Larval Density Derived Metamorph Prediction)",
       ...
       title = "2023")+
  theme(plot.title = element_text(size = 30), axis.text = element_text(size = 16), axis.title = element_text(size = 18))+
  annotate("text",x = 0.1, y = 6, label = paste("R² = ", round(summary2023$r.squared, 3), "\n", "p = ", round(p_value2023, 3)),
           size = 5, color = "black")
...
plot4 <- ggplot(data2024, aes(x = log(Metamorph_Estimate+1), y = log(Drift_Fence_Actual_Count))) +
  ...
  labs(x = "log(Larval Density Derived Metamorph Prediction)",
       ...
       title = "2024")+
  theme(plot.title = element_text(size = 30), axis.text = element_text(size = 16), axis.title = element_text(size = 18))+
  annotate("text",  x = 0.1, y = 7, label = paste("R² = ", round(summary2024$r.squared, 3), "\n", "p = ", round(p_value2024, 3)),
           size = 5, color = "black")
...
ggplot(data2022NoCalabasas, aes(x = log(Metamorph_Estimate+1), y = log(Drift_Fence_Actual_Count+1))) +
  geom_point() +
  stat_smooth(method = "lm", color = "black")+
  labs(x = "log(Larval Density Derived Metamorph Prediction)",
       y = "log(Actual Metamorph Count)",
  ...
  theme(plot.title = element_text(size = 30), axis.text = element_text(size = 16), axis.title = element_text(size = 18))+
  annotate("text", x = 0.01, y = 6.65, label = paste("R² = ", round(summary2022NoCalabasas$r.squared, 3), "\n", "p = ", round(p_value2022NoCalabasas, 3)),
           size = 5, color = "black")
